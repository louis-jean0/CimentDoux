...
class RigidBody {
    public:
        ...
        RigidBody() = default;
        RigidBody(std::weak_ptr<SceneNode> node) : node(node), velocity(0.0f), mass(1.0f), use_gravity(false), is_on_ground(false), friction_coefficient(100.0f), restitution_coefficient(0.0f) {}
        ~RigidBody() = default;
